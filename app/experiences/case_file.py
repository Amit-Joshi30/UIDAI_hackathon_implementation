...
"""
# Updated: 2026-01-20 19:53 IST - USING COMPONENTS.HTML
...
def render_case_file(record: Optional[Dict] = None, policy_record: Optional[Dict] = None):
    ...
    if is_desert:
        desert_badge = f"""
        <span style="background: rgba(248, 81, 73, 0.2); color: #F85149;
        padding: 4px 10px; border-radius: 4px; font-size: 0.7rem;
        font-weight: 600; letter-spacing: 0.05em; margin-left: 12px;">
        SERVICE DESERT
        </span>
        """
    ...
    if priority_rank and priority_rank <= 100:
        rank_badge = f"""
        <span style="background: rgba(255, 153, 51, 0.2); color: #FF9933;
        padding: 4px 10px; border-radius: 4px; font-size: 0.7rem;
        font-weight: 600; letter-spacing: 0.05em; margin-left: 8px;">
    ...
    if mismatch_type:
        ...
        <div style="padding: 1rem 1.5rem; border-bottom: 1px solid rgba(255, 255, 255, 0.1);
            ...
            <p style="color: #E6EDF3; font-size: 0.95rem; margin: 0.25rem 0 0 0;">
                {mismatch_type}
            </p>
            <p style="color: #6E7681; font-size: 0.7rem; margin: 0.25rem 0 0 0;">
    ...
    if composite_priority is not None:
        composite_block = f"""
        <div style="padding: 0.75rem 1.5rem; background: rgba(0, 0, 0, 0.3);
            ...
            <span style="color: #1A73E8; font-size: 1.1rem; font-weight: 700;
                         font-family: 'SF Mono', Consolas, monospace;">
                {composite_priority:.4f}
            </span>
        </div>
        """
...
def render_case_file_placeholder():
    st.markdown(
        ...
        <div style="background: rgba(22, 27, 34, 0.6);
            ...
            <p style="color: #8B949E; font-size: 1rem; margin: 0 0 0.5rem 0;">
                Pincode Metrics
            </p>
            <p style="color: #6E7681; font-size: 0.85rem; margin: 0;">
                Enter a 6-digit pincode in the sidebar to view artifact data
            </p>
        </div>
        """,
...
def render_case_file_header():
    st.markdown(
        """
        <h2 style="color: #E6EDF3; font-size: 1.25rem; font-weight: 600;
                   margin: 2rem 0 0.5rem 0;">
            Pincode Metrics
        </h2>
        <p style="color: #8B949E; font-size: 0.85rem; margin: 0 0 1rem 0;">
            Data sourced from pincode_aggregates.csv and policy_recommendations.csv
        </p>
        """,
        unsafe_allow_html=True,
    )
